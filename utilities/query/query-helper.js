...
module.exports = {
	 ...
	 */
	createMongooseQuery: function (model, query, mongooseQuery) {
		...
		mongooseQuery = this.setExclude(query, mongooseQuery);

		let attributesFilter = this.createAttributesFilter(query, model);
		if (attributesFilter === '') {
			attributesFilter = "_id";
		}

		let result = this.populateEmbeddedDocs(query, mongooseQuery, attributesFilter, model);
		mongooseQuery = result.mongooseQuery;
		attributesFilter = result.attributesFilter;

		mongooseQuery = this.setSort(query, mongooseQuery);
		...
		for (let fieldQueryKey in query) {
			...
			let attributesArray = attributesFilter.split(' ');
			if (_.includes(attributesArray, fieldQueryKey)) {
				if (!Array.isArray(fieldQuery)) {
					fieldQuery = [];
					fieldQuery.push(query[fieldQueryKey]);
				}
		...
		this.setTermSearch(query, model);
	...
	createAttributesFilter: function (query, model) {
		...
		let relations = SchemaUtility.relationsFromSchema(model.modelName, 1, 2);
